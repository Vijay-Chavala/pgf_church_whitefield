...
export function GalleryOverview() {
  ...
  // Get sample images from subcategories for preview
  const getCategoryPreviewImages = (categoryId: string): string[] => {
    ...
    category.subcategories.forEach(subcategory => {
      if (images.length < 4) {
        images.push(subcategory.coverImage)
        // Add more images from the subcategory if needed
        subcategory.images.slice(0, 4 - images.length).forEach(img => {
          if (images.length < 4) images.push(img.src)
        })
      }
    })

    // Ensure we have at least 4 images, fill with duplicates if needed
